...
angular.module('myApp')
    .controller('indexRD_Controller',['$rootScope','constant','$location','$scope','$filter','loginService','$state','importService','$cookies',
        function($rootScope,constant,$location,$scope,$filter,loginService,$state,importService,$cookies) {
            ...
            $scope.file = {};
            $scope.file.fugai = false;
            $scope.importCategories = ['保单导入','潜客导入'];
            $scope.importCategoriesNew = ['保单导入','潜客导入','销售战败线索'];
            ...
            $scope.openWindow = function (){
                ...
                $("#sleepBatch").click(function(){
                    $("#plsm").show();
                });
                $scope.setPageStatus('qkcx',0,'');
            }

            //传页面类型给子页面
            $scope.setPageStatus = function(pageTypeStatus,pageStatus,pageTZ){
                if(pageTZ=='bd'){
                    $state.go('renewalDirector.policy');
                }else if(pageTZ=='qk'){
                    $state.go('renewalDirector.customer',{reload:true});
                }
                setTimeout(function(){
                    if($state.current.url=="/customerRD"){
            ...
            $scope.changeFile = function(){
                ...
                if($scope.file.importCategory){
                    if($scope.file.importCategory == '潜客导入' && $scope.file.fugai == true){
                        isFugai = 1;
                    }
                    importService.upload($scope.file.fileData,$scope.file.importCategory,isFugai).then(function(res){
                        $scope.show = false;
                        $scope.message = res.results.message;
                        if(res.results.content.errorMessage){
                            $scope.showLink = true;
                        }else {
                            $scope.showLink = false;
                        }
                        $scope.file.fileData = null;
                        $scope.findCountByUserIdTop();
                        $scope.$broadcast('shuaXinYeMian',true);
                    });
                }else{
                    $scope.show = false;
                    $scope.message = "请选择导入类别";
                    $scope.file.fileData = null;
                    $scope.showLink = false;
                }
            };
            ...
            $scope.sleepBatch = function(){
                ...
                //if($scope.file.importCategory){
                    importService.sleepBatch($scope.file.sleepData).then(function(res){
                        $scope.show = false;
                        $scope.message = res.results.message;
                        if(res.results.content.errorMessage){
                            $scope.showLink = true;
