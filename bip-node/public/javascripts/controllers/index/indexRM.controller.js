...
angular.module('myApp')
    .controller('indexRM_Controller',['$rootScope','constant','$location','$scope','$filter','loginService','$state','$cookies',
        function($rootScope,constant,$location,$scope,$filter,loginService,$state,$cookies) {
           ...
           $scope.findCountByUserIdTop();
            ...
            $scope.setPageStatus = function(pageTypeStatus,pageStatus,pageTZ){
                if(pageTZ=='bd'){
                    $state.go('receptionManager.policy');
                }else if(pageTZ=='qk'){
                    $state.go('receptionManager.customer',{reload:true});
                }
                setTimeout(function(){
                    if($state.current.url=="/customerRM"){
