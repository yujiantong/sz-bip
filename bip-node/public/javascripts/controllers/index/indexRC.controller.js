...
angular.module('myApp')
    .controller('indexRC_Controller',['$rootScope','constant','$location','$scope','$filter','loginService','$state','$cookies','customerIWService',
        function($rootScope,constant,$location,$scope,$filter,loginService,$state,$cookies,customerIWService) {
            ...
            $scope.setPageStatus = function(pageTypeStatus,pageStatus,pageTZ){
                if(pageTZ=='bd'){
                    $state.go('renewalCommissioner.policy');
                }else if(pageTZ=='qk'){
                    $state.go('renewalCommissioner.customer',{reload:true});
                }
                setTimeout(function(){
                    if($state.current.url=="/customerRC"){
            ...
            $scope.geRenXingXi=function(customer,clickFlag){
                if($state.current.url!="/customerRC"){
                    $state.go('renewalCommissioner.customer',{reload:true});
                    var timer = setInterval(function(){
                        if($scope.cycle == true){
                            clearInterval(timer);
                            $scope.$broadcast('geRenXingXi',customer);
                            $scope.$broadcast('clickFlag',clickFlag);
                        }
                    },1000)
            ...
            $scope.geRenXXClose=function(){
                $scope.$broadcast('pageTypeOld',true);
                $("#notice").hide();
            };
            //信息弹出框
