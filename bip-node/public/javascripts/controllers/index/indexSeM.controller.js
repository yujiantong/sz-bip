...
angular.module('myApp')
    .controller('indexSeM_Controller',['$rootScope','constant','$location','$scope','$filter','loginService','importService','$state','$cookies',
        function($rootScope,constant,$location,$scope,$filter,loginService,importService,$state,$cookies) {
            ...
            $scope.file = {};
            $scope.importCategories = ['推送修','维修记录','工时记录','配件记录'];
            ...
            //根据4s店id查询保险公司信息
            loginService.findCompInfoByStoreId().then(function(res){
            ...
            $scope.setPageStatus = function(pageTypeStatus,pageStatus,pageTZ){
                if(pageTZ=='bd'){
                    $state.go('serviceManager.policy');
                }else if(pageTZ=='qk'){
                    $state.go('serviceManager.customer',{reload:true});
                }
                setTimeout(function(){
                    if($state.current.url=="/customerSeM"){
            ...
            $scope.changeFile = function(){
                ...
                if($scope.file.importCategory){
                    if($scope.file.importCategory=='推送修'){
                        if(!$scope.file.insuranceCompName){
                            $scope.show = false;
                            $scope.message = "请选择保险公司";
                            $scope.file.fileData = null;
                            return;
                        }
                    }
                    importService.tsxImport($scope.file.fileData,$scope.file.importCategory,$scope.file.insuranceCompName).then(function(res){
                        $scope.show = false;
