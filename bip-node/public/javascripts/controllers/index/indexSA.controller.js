...
angular.module('myApp')
    .controller('indexSA_Controller',['$rootScope','constant','$location','$scope','$filter','loginService','$state','$cookies',
        function($rootScope,constant,$location,$scope,$filter,loginService,$state,$cookies) {
            ...
            $scope.setPageStatus = function(pageTypeStatus,pageStatus,pageTZ){
                if(pageTZ=='bd'){
                    $state.go('serviceAdvisor.policy');
                }else if(pageTZ=='qk'){
                    $state.go('serviceAdvisor.customer',{reload:true});
                }
                setTimeout(function(){
                    if($state.current.url=="/customerSA"){
