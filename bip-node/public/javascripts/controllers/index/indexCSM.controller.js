...
angular.module('myApp')
    .controller('indexCSM_Controller',['$rootScope','constant','$location','$scope','$filter','loginService','importService','$state','$cookies',
        function($rootScope,constant,$location,$scope,$filter,loginService,importService,$state,$cookies) {
            ...
            $scope.setPageStatus = function(pageTypeStatus,pageStatus,pageTZ){
                if(pageTZ=='qk'){
                    $state.go('customerServiceManager.customer',{reload:true});
                }
                setTimeout(function(){
                    if($state.current.url=="/customerCSM"){
            ...
            //销售战败潜客导入
            $scope.importDefeat = function(){
                $scope.show = true;
                $scope.showLink = false;
                if($scope.message){
                    $scope.message = null;
                }
                importService.importDefeat($scope.file.defeatData).then(function(res){
                    $scope.show = false;
