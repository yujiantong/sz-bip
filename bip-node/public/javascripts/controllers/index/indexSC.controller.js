...
angular.module('myApp')
    .controller('indexSC_Controller',['$rootScope','constant','$location','$scope','$filter','loginService','$state','$cookies',
        function($rootScope,constant,$location,$scope,$filter,loginService,$state,$cookies) {
            ...
            $scope.setPageStatus = function(pageTypeStatus,pageStatus,pageTZ){
                if(pageTZ=='bd'){
                    $state.go('salesConsultant.policy');
                }else if(pageTZ=='qk'){
                    $state.go('salesConsultant.customer',{reload:true});
                }
                setTimeout(function(){
                    if($state.current.url=="/customerSC"){
