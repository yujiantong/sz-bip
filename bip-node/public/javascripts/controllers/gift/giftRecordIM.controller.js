...
angular.module('myApp')
    .controller('giftRecordIM_Controller',['$rootScope','$scope','$filter','giftRecordIMService',
        function($rootScope,$scope,$filter,giftRecordIMService,$state) {
            $scope.customerAll = [];
            ...
            $scope.findGivingCont = function(){
                ...
                var insuranceWriter = $scope.hxSearch.insuranceWriter;
                ...
                if($scope.startNum==1) {
                    $scope.condition = {
                        ...
                        createTimeStart:createTimeStart,createTimeEnd:createTimeEnd,insuranceWriter:insuranceWriter
                    };
                }else{
                    $scope.condition.startNum = $scope.startNum;
                };
                $("#msgwindow").show();
                giftRecordIMService.findGivingByCondition($scope.condition)
            ...
            $scope.giftDetails=function(rowData){
                ...
                $scope.custInfo.zhyh = $scope.custInfo.privilegeSum+$scope.custInfo.donateCostCount;
                $scope.approvalBillId = rowData.approvalBillId;
                $("#msgwindow").show();
                giftRecordIMService.findGivingByApprovalBillId($scope.approvalBillId).then(function (result) {
                    $("#msgwindow").hide();
                    $("#hxdetail").show();
                    if (result.status == 'OK'&&result.results.content.status=='OK') {
            ...
            //查询本店各种用户
            giftRecordIMService.findKindsUser().then(function(res){
                ...
                var creatingFile =$scope.$watch("loadDataFinish", function(newValue, oldValue) {
                    ...
                    if ($scope.loadDataFinish) {
                        ...
                        setTimeout(function(){
                            ...
                            giftRecordIMService.exportGiftRecordExcel($scope.exportCustomerData,$scope.exportData);
                    ...
                    .then(function (result) {
                        ...
                        if (result.status == 'OK' && result.results.content.status == 'OK') {
                            ...
                            if(result.results.content.results.length > 0){
                                ...
                                giftRecordIMService.exportGiftRecord(condition).then(function (resultCustomer){
            ...
            $scope.getExportDataCallback = function(customerArray,policyExportCount){
                ...
                if(customerArray.length<$rootScope.pageSize){
                    ...
                    console.log("最终导出信息" + JSON.stringify($scope.exportData));
                    //加密手机号 contactWay
                    angular.forEach($scope.exportData, function(data,index,array){
                        array[index].contactWay = turnTelF(array[index].contactWay );
                    });
                }
