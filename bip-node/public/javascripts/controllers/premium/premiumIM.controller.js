...
angular.module('myApp')
    .controller('premiumIM_Controller',['$rootScope','$scope','$filter','premiumIMService',
        function($rootScope,$scope,$filter,premiumIMService) {
            $scope.premiumAll = [];
            ...
            //根据4s店id查询保险公司信息
            premiumIMService.findCompInfoByStoreId().then(function(res){
            ...
            //查询本店各种用户
            premiumIMService.findKindsUser().then(function(res){
                if(res.status == 'OK'&&res.results.content.status=='OK'){
            ...
            $scope.findAllpremiumByUserId = function(){
                ...
                premiumIMService.findInformationAndPremiumCount(condition).then(function (result) {
            ...
            $scope.giftDetails=function(rowData){
                ...
                $("#msgwindow").show();
                premiumIMService.findGivingByApprovalBillId($scope.approvalBillId).then(function (result) {
                    $("#msgwindow").hide();
                    $("#bfdetail").show();
                    if (result.status == 'OK'&&result.results.content.status=='OK') {
