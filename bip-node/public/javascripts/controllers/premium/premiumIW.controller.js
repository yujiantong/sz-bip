...
angular.module('myApp')
    .controller('premiumIW_Controller',['$rootScope','$scope','$filter','premiumIWService',
        function($rootScope,$scope,$filter,premiumIWService) {
            $scope.premiumAll = [];
            ...
            //根据4s店id查询保险公司信息
            premiumIWService.findCompInfoByStoreId().then(function(res){
            ...
            //查询本店各种用户
            premiumIWService.findKindsUser().then(function(res){
                if(res.status == 'OK'&&res.results.content.status=='OK'){
            ...
            $scope.findAllpremiumByUserId = function(){
                ...
                var condition = {
                    ...
                    chassisNumber:chassisNumber,cbrqStart:cbrqStart,cbrqEnd:cbrqEnd,start:start
                }
                premiumIWService.findInformationAndPremiumCount(condition).then(function (result) {
            ...
            $scope.giftDetails=function(rowData){
                ...
                $("#msgwindow").show();
                premiumIWService.findGivingByApprovalBillId($scope.approvalBillId).then(function (result) {
                    $("#msgwindow").hide();
                    $("#bfdetail").show();
                    if (result.status == 'OK'&&result.results.content.status=='OK') {
