...
angular.module('myApp')
    .controller('maintainRecordSeM_Controller',['$rootScope','$scope','$filter','maintainRecordSeMService','$state',
        function($rootScope,$scope,$filter,maintainRecordSeMService,$state) {
            $scope.customerAll = [];
            ...
            //查询持有人列表
            maintainRecordSeMService.selectUserForHolderSearch().then(function(res){
            ...
            $scope.findwxRecordByCondition = function(){
                ...
                var consultantName = $scope.wxSearch.consultantName;
                var maintenanceType = $scope.wxSearch.maintenanceType;
                ...
                if($scope.startNum==1) {
                    $scope.condition = {
                        ...
                        maintenanceTimeEndEnd:maintenanceTimeEndEnd, bdcb:bdcb, consultantName:consultantName,
                ...
                $("#msgwindow").show();
                maintainRecordSeMService.findwxRecordByCondition($scope.condition)
            ...
            $scope.maintainDetails=function(maintainNumber){
                ...
                $("#msgwindow").show();
                maintainRecordSeMService.findByMaintainNumber($scope.maintainNumber).then(function (result) {
                    $("#msgwindow").hide();
                    if (result.status == 'OK'&&result.results.content.status=='OK') {
                        $("#maintainDetail").show();
