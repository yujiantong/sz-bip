...
angular.module('myApp')
    .controller('policyGM_Controller',['$rootScope','$location','constant','$scope','$filter','policyGMService','$state','ExportExcel',
        function($rootScope,$location,constant,$scope,$filter,policyGMService,$state,ExportExcel){
            ...
            //下拉列表数据初始化
            //按4s店ID查询车辆品牌车型信息
            policyGMService.findCarInfoByStoreId().then(function(res){
            ...
            //根据4s店id查询保险公司信息
            policyGMService.findCompInfoByStoreId().then(function(res){
            ...
            //查询本店各种用户
            policyGMService.findKindsUser().then(function(res){
                if(res.status == 'OK'&&res.results.content.status=='OK'){
            ...
            $scope.policyAllPage = {
                ...
                columnDefs: [
                    ...
                    { name: '出单员',  enableColumnMenu: false,width:100,allowCellFocus : false,enableSorting: false,
                        ...
                        '<div class="rowButtons" style="width:200px;right:-200px">'+
            ...
            $scope.getSingleByCovertype = function(covertype){
                ...
                $scope.policyAll.data=[];
                $("#msgwindow").show();
                policyGMService.getSingleAll(covertype,$scope.startTime,$scope.endTime,$scope.startNum,$scope.showAll,short,shortmain)
            ...
            $scope.getSingleByCovertypeTime = function(){
                ...
                if(!(re_data.test($scope.startTime)) || !(re_data.test($scope.endTime))){
                    ...
                    $("#tipAlert").hide();
                    $("#msgwindow").show();
                    policyGMService.getSingleAll($scope.covertype,$scope.startTime,$scope.endTime,$scope.startNum,$scope.showAll)
                        .then(function (result) {
                            $("#msgwindow").hide();
                            if (result.status == 'OK') {
                                $scope.policyAll.data = result.result.content.results;
            ...
            $scope.floatingWindow=function(){
                ...
                $('.ui-grid-row').mouseleave(function(){
                    $(this).find(".rowButtons").stop(true).animate({right:'-200px'},100);
                });
            ...
            $scope.singleDetails=function(insuranceBillId) {
                ...
                $scope.searchPolicy = {};
                $("#msgwindow").show();
                policyGMService.getSingleById(insuranceBillId).then(function (result) {
                    $("#msgwindow").hide();
            ...
            $scope.findRecord=function(insuranceBillId) {
                ...
                policyGMService.findRecord(insuranceBillId).then(function (result) {
            ...
            $scope.singleSearch=function(){
                ...
                if(cbrqStart != null && cbrqEnd != null && cbrqStart != "" && cbrqEnd != "" && cbrqStart>cbrqEnd){
                    ...
                    $("#msgwindow").show();
                    policyGMService.getSingleByCondition(searchDatas,$scope.startNum2).then(function(result){
                        $("#msgwindow").hide();
                        if(result.status == 'OK'){
            ...
            $scope.deleteBill = function(obj){
                ...
                $scope.makesure = function() {
                    $("#bdsc").hide();
                    $("#msgwindow").show();
                    policyGMService.deleteBill(obj.insuranceBillId).then(function (result) {
                        $("#msgwindow").hide();
                        if (result.status == 'OK') {
            ...
            //查询所有险种
            policyGMService.findInsu().then(function (result) {
                if (result.status == 'OK') {
            ...
            $scope.singleSearchExport=function(){
                ...
                if(cbrqStart != null && cbrqEnd != null && cbrqStart != "" && cbrqEnd != "" && cbrqStart>cbrqEnd){
                    ...
                    policyGMService.getSingleByCondition(searchDatas,$scope.startNumExport).then(function(result){
            ...
            $scope.getSingleByCovertypeExport = function(covertype){
                ...
                $scope.covertype = covertype;
                $scope.policyAll.data=[];
                policyGMService.getSingleAll(covertype,$scope.startTime,$scope.endTime,$scope.startNumExport,$scope.showAll)
                    .then(function (result) {
                        if (result.status == 'OK') {
                            $scope.getExportDataCallback(result.result.content.results,result.result.content.policyCount);
                        } else {
                        ...
                        for(var j=0;j<$scope.insuranceAll.length;j++){
                            ...
                            for(var k=0;k<customerArray[i].insuTypes.length;k++){
                                ...
                                if($scope.insuranceAll[j].typeName==customerArray[i].insuTypes[k].typeName){
                                    ...
                                    }else if(customerArray[i].insuTypes[k].typeName.indexOf("三者") > -1){
                                    ...
                                    }else  if(customerArray[i].insuTypes[k].typeName.indexOf("精神") > -1){
                                        arr = {checkState:true,coverage:jingshen,typeName:$scope.insuranceAll[j].typeName};
                                    }else  if(customerArray[i].insuTypes[k].typeName.indexOf("货物") > -1){
                                        arr = {checkState:true,coverage:huowu,typeName:$scope.insuranceAll[j].typeName};
                                    }else {
