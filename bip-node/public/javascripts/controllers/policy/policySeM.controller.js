...
angular.module('myApp')
    .controller('policySeM_Controller',['$rootScope','$scope','$filter','policySeMService','$state',
        function($rootScope,$scope,$filter,policySeMService,$state){
            ...
            //下拉列表数据初始化
            //按4s店ID查询车辆品牌车型信息
            policySeMService.findCarInfoByStoreId().then(function(res){
            ...
            //根据4s店id查询保险公司信息
            policySeMService.findCompInfoByStoreId().then(function(res){
            ...
            //查询本店各种用户
            policySeMService.findKindsUser().then(function(res){
                if(res.status == 'OK'&&res.results.content.status=='OK'){
            ...
            $scope.getSingleByCovertype = function(covertype){
                ...
                $scope.policyAll.data=[];
                $("#msgwindow").show();
                policySeMService.getSingleAll(covertype,$scope.startTime,$scope.endTime,$scope.startNum,$scope.showAll,short,shortmain)
            ...
            $scope.getSingleByCovertypeTime = function(){
                ...
                if(!(re_data.test($scope.startTime)) || !(re_data.test($scope.endTime))){
                    ...
                    $("#tipAlert").hide();
                    $("#msgwindow").show();
                    policySeMService.getSingleAll($scope.covertype,$scope.startTime,$scope.endTime,$scope.startNum,$scope.showAll)
                        .then(function (result) {
                            $("#msgwindow").hide();
                            if (result.status == 'OK') {
                                $scope.policyAll.data = result.result.content.results;
            ...
            $scope.singleDetails=function(insuranceBillId) {
                ...
                $scope.searchPolicy = {};
                $("#msgwindow").show();
                policySeMService.getSingleById(insuranceBillId).then(function (result) {
                    $("#msgwindow").hide();
            ...
            $scope.findRecord=function(insuranceBillId) {
                ...
                policySeMService.findRecord(insuranceBillId).then(function (result) {
            ...
            $scope.singleSearch=function(){
                ...
                if(cbrqStart != null && cbrqEnd != null && cbrqStart != "" && cbrqEnd != "" && cbrqStart>cbrqEnd){
                    ...
                    $("#msgwindow").show();
                    policySeMService.getSingleByCondition(searchDatas,$scope.startNum2).then(function(result){
                        $("#msgwindow").hide();
                        if(result.status == 'OK' ){
