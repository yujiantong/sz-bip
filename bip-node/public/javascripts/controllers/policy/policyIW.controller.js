...
angular.module('myApp')
    .controller('policyIW_Controller',['$rootScope','$scope','$filter','policyIWService','$state',
        function($rootScope,$scope,$filter,policyIWService,$state,customers){
            ...
            $scope.sleepCustomerPromptFlag = 0;//0表示没提示过,1表示提示过
            $scope.searchsingle={};
            ...
            $scope.startNum2 = 1;
            $scope.loadStatus=2;
            $scope.loadStatus2=2;//判定数据是否加载完成。  ---默认为加载完成
            $scope.showAll = false;
            ...
            //根据4s店id查询保险公司信息
            policyIWService.findCompInfoByStoreId().then(function(res){
            ...
            //查询本店各种用户
            policyIWService.findKindsUser().then(function(res){
                if(res.status == 'OK'&&res.results.content.status=='OK'){
            ...
            $scope.policyAllPage = {
                ...
                columnDefs: [
                    ...
                    '<div role="button" class="ui-grid-cell-contents fl" ng-click="grid.appScope.qjsort_xb(6)" style="width: 110px;"><span>车架号</span></div><div class="fr">' +
            ...
            $scope.getSingleByCovertype = function(covertype){
                ...
                $scope.policyAll.data=[];
                $("#msgwindow").show();
                policyIWService.getSingleAll(covertype,$scope.startTime,$scope.endTime,$scope.startNum,$scope.showAll,short,shortmain)
            ...
            $scope.getSingleByCovertypeTime = function(){
                ...
                if(!(re_data.test($scope.startTime)) || !(re_data.test($scope.endTime))){
                    ...
                    $("#tipAlert").hide();
                    $("#msgwindow").show();
                    policyIWService.getSingleAll($scope.covertype,$scope.startTime,$scope.endTime,$scope.startNum,$scope.showAll)
                        .then(function (result) {
                            $("#msgwindow").hide();
                            if (result.status == 'OK') {
                                $scope.policyAll.data = result.result.content.results;
            ...
            $scope.floatingWindow=function(){
                ...
                $('.ui-grid-row').mouseenter(function(){
                    $(this).find(".rowButtons").stop(true).animate({right:'-60px'},500);
                });
                $('.ui-grid-row').mouseleave(function(){
            ...
            $scope.singleDetails=function(rowData) {
               ...
               /* var newDate_M =(newDate.getMonth()+1 < 10 ? '0' +(newDate.getMonth()+1) : newDate.getMonth()+1);
                if(rowData.foundDate!=null && rowData.foundDate!=''){
                    var foundDate = new Date(rowData.foundDate);
                    var foundDate_M = (foundDate.getMonth()+1 < 10 ? '0' +(foundDate.getMonth()+1) : foundDate.getMonth()+1);
                }
                var between = (newDate - rowData.foundDate)/1000;
                if(between<=86400&& newDate_M==foundDate_M){
                    $scope.foundDate = 1;
                }else if(newDate_M!=foundDate_M){
                    $scope.foundDate = 0;
                }else {
                    $scope.foundDate = 0;
                }
                */
                ...
                $("#msgwindow").show();
                policyIWService.getSingleById(insuranceBillId).then(function (result) {
                    $("#msgwindow").hide();
            ...
            $scope.findRecord=function(insuranceBillId) {
                ...
                policyIWService.findRecord(insuranceBillId).then(function (result) {
                    if (result.status=='OK'&&result.results.content.status == 'OK') {
            ...
            $scope.policyAllSearchPage = {
                ...
                columnDefs: [
                    ...
                    { name: '车损投保金额', field: 'chesuntbje',width:90,enableColumnMenu: false},
                    { field: 'chassisNumber',enableSorting: false,cellTooltip: true, enableColumnMenu: false,minWidth:150,headerCellTemplate: '' +
                    '<div role="button" class="ui-grid-cell-contents fl" ng-click="grid.appScope.qjsort_bd(6)" style="width: 110px;"><span>车架号</span></div><div class="fr">' +
                    '<span class="slick-header-button" ng-click="grid.appScope.newPolicybtn();"></span></div>'},
            ...
            $scope.singleSearch=function(){
                ...
                if(cbrqStart != null && cbrqEnd != null && cbrqStart != "" && cbrqEnd != "" && cbrqStart>cbrqEnd){
                    ...
                    $("#msgwindow").show();
                    policyIWService.getSingleByCondition(searchDatas,$scope.startNum2).then(function(result){
                        $("#msgwindow").hide();
           ...
           /* $scope.sanzheCk = function(){
                console.log($scope.sanzhe);
            } ;
            $scope.sijiCk = function(){
                console.log($scope.SiJick);
            };
            $scope.chengkeCk = function(){
                console.log($scope.ChengKe);
            };
            $scope.boliCk = function(){
                console.log($scope.BoLi);
            };
            $scope.huahenCk = function(){
                console.log($scope.HuaHen);
            };
            $scope.feiyongbuchangCk = function(){
                console.log($scope.hcFeiYongBuChang);
            };
            $scope.xiulibuchangCk = function(){
                console.log($scope.hcXiuLiBuChang);
            };
            $scope.jingshenCk = function(){
                console.log($scope.HcJingShenSunShi);
            };
            $scope.huowuCk = function(){
                console.log($scope.HcHuoWuZeRen);
            };*/
            //险种选中变状态
            ...
            //新增保单品牌车型选择框与输入框切换
            $scope.xzVehicleModelbd = function(){
                $scope.searchsingle.vehicleModel='';
                $scope.searchsingle.vehicleModelInput='';
                if($scope.searchsingle.carBrand){
                    if($scope.searchsingle.carBrand.brandName=='异系'){
                        $("#clxhsr_xbd2").show();
                        $("#clxhxz_xbd2").hide();
                    }else{
                        $("#clxhxz_xbd2").show();
