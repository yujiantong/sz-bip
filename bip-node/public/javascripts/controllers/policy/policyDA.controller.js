...
angular.module('myApp')
    .controller('policyDA_Controller',['$rootScope','$scope','$filter','policyDAService','$state',
        function($rootScope,$scope,$filter,policyDAService,$state){
            ...
            //下拉列表数据初始化
            //按4s店ID查询车辆品牌车型信息
            policyDAService.findCarInfoByStoreId().then(function(res){
            ...
            //根据4s店id查询保险公司信息
            policyDAService.findCompInfoByStoreId().then(function(res){
            ...
            //查询本店各种用户
            policyDAService.findKindsUser().then(function(res){
                if(res.status == 'OK'&&res.results.content.status=='OK'){
            ...
            $scope.getSingleByCovertype = function(covertype){
                ...
                $scope.policyAll.data=[];
                $("#msgwindow").show();
                policyDAService.getSingleAll(covertype,$scope.startTime,$scope.endTime,$scope.startNum,$scope.showAll,short,shortmain)
            ...
            $scope.getSingleByCovertypeTime = function(){
                ...
                if(!(re_data.test($scope.startTime)) || !(re_data.test($scope.endTime))){
                    ...
                    $("#tipAlert").hide();
                    $("#msgwindow").show();
                    policyDAService.getSingleAll($scope.covertype,$scope.startTime,$scope.endTime,$scope.startNum,$scope.showAll)
                        .then(function (result) {
                            $("#msgwindow").hide();
                            if (result.status == 'OK') {
                                $scope.policyAll.data = result.result.content.results;
            ...
            $scope.singleDetails=function(insuranceBillId) {
                ...
                $scope.searchPolicy = {};
                $("#msgwindow").show();
                policyDAService.getSingleById(insuranceBillId).then(function (result) {
                    $("#msgwindow").hide();
            ...
            $scope.findRecord=function(insuranceBillId) {
                ...
                policyDAService.findRecord(insuranceBillId).then(function (result) {
            ...
            $scope.singleSearch=function(){
                ...
                if(cbrqStart != null && cbrqEnd != null && cbrqStart != "" && cbrqEnd != "" && cbrqStart>cbrqEnd){
                    ...
                    $("#msgwindow").show();
                    policyDAService.getSingleByCondition(searchDatas,$scope.startNum2).then(function(result){
                        $("#msgwindow").hide();
                        if(result.status == 'OK'){
            ...
            $scope.spdSearchPage = {
                ...
                columnDefs: [
                    ...
                    { name: '保险公司',  enableColumnMenu: false,width:100,allowCellFocus : false,enableSorting: false,
                        ...
                        '<div class="rowButtons" style="width:100px;right:-100px">'+
            ...
            $scope.spdSearchBtn=function(){
                ...
                $("#msgwindow").show();
                policyDAService.getSpdByCondition(spdDatas,$scope.startNum3,billFlag,short,shortmain).then(function(result){
                    $("#msgwindow").hide();
                    if(result.status == 'OK'&&result.results.success==true){
            ...
            $scope.SpdDetails=function(id,flag) {
                ...
                if(id){
                    $scope.spddetail = {};
                    $("#msgwindow").show();
                    policyDAService.getSpdById(id,billFlag).then(function (result) {
                        $("#msgwindow").hide();
                        if (result.status=='OK'&&result.results.content.status == 'OK') {
                            ...
                            $scope.angularTip("审批单明细查询失败",5000);
                        }
                    });
                }else {
