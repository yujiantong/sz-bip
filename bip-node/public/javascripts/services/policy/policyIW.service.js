...
angular.module('myApp')
    .service('policyIWService', ['$http','$q', function($http,$q){
        //按承保类型查询保单信息
        ...
        //按车架号校验本月是否已出单
        this.findExistBillThisMonth = function(chassisNumber,insurDate){
            var deferred = $q.defer();
            $http.post('/insurance/findExistBillThisMonth',{chassisNumber:chassisNumber,insurDate:insurDate})
                .success(function(result){
                    deferred.resolve(result);
                })
                .error(function(result){
        ...
        //提交新增保单信息
        this.addSingle = function(newsingle,traceRecord,approvalBillAll,identifying,insuTypes,insuredQuota){
            console.log("输出参数 " + JSON.stringify(newsingle.insuranceType) + " " + JSON.stringify(insuredQuota))
            if(newsingle.insuranceType && verificationInsuranceType(newsingle.insuranceType)){
                var insuranceType = newsingle.insuranceType.split(",");
                var insuranceTypes = "";
                ...
                newsingle.insuranceType = insuranceTypes.substr(0 , insuranceTypes.length-1)
                console.log("insuranceTypes ==>" + JSON.stringify(newsingle.insuranceType))
            }
            //newsingle.insuranceType
            var deferred = $q.defer();
            $http.post('/insurance/addInsurance',{insureInfo:JSON.stringify(newsingle),traceRecord:JSON.stringify(traceRecord),
                approvalBillAll:JSON.stringify(approvalBillAll),identifying:identifying,insuTypes:JSON.stringify(insuTypes)})
                // $http.get('pcList.json')
                .success(function(result){
                    deferred.resolve(result);
                })
        ...
        //调用博福接口获取车辆已投保信息
        this.getCarInsuranceInfo = function(param){
            var deferred = $q.defer();
            $http.post('/bihu/getCarInsuranceInfo',{condition:JSON.stringify(param)})
                // $http.get('pcList.json')
                .success(function(result){
                    deferred.resolve(result);
                })
