...
angular.module('myApp')
    .service('policySeMService', ['$http','$q', function($http,$q){
        //按承保类型查询保单信息
        this.getSingleAll = function(coverType,startTime,endTime,startNum,showAll,short,shortmain){
            var deferred = $q.defer();
            $http.post('/insurance/findSeMInsuranceBill',{coverType:coverType,startTime:startTime,endTime:endTime,
        ...
        this.getSingleByCondition = function(searchDatas,startNum){
            var deferred = $q.defer();
            $http.post('/insurance/findAllSeMInsuranceBill',{condition:JSON.stringify(searchDatas),startNum:startNum})
                // $http.get('pcList.json')
                .success(function(result){
                    deferred.resolve(result);
                })
