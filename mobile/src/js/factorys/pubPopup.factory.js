...
function($ionicTemplateLoader, $ionicBackdrop, $q, $timeout, $rootScope, $ionicBody, $compile, $ionicPlatform, $ionicModal, IONIC_BACK_PRIORITY) {
  ...
  '<div class="popup-container" ng-class="cssClass">' +
    '<div class="popup">' +
      // '<div class="tWindow">' +
        '<div class="tWindowTitle" ng-bind-html="title"></div>'+
        '<div class="tWindowContents">' +
          '<div class="tWindowText">' +
            '<div class="tWindowT"></div>' +
          '</div>' +
        '</div>' +
        '<div class="tWindowBottoms" ng-show="buttons.length">' +
          '<div class="tWindowLine"></div>' +
          '<div class="row row-no-padding" style="text-align:center;">' +
            '<div class="col col-center hotlink" ng-class="button.type" ng-repeat="button in buttons" ng-click="$buttonTapped(button, $event)" ng-bind-html="button.text">' +
            '</div>' +
          '</div>' +
        '</div>' +
      // '</div>'+
  ...
  function createPopup(options) {
    ...
    $q.when(
      ...
      var popupBody = angular.element(self.element[0].querySelector('.tWindowT'));
      if (template) {
  ...
  function showConfirm(opts) {
    return showPopup(angular.extend({
      buttons: [{
        ...
        type: opts.cancelType || 'warningLink',
