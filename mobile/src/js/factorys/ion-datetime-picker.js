angular.module("starter")
    .factory("$ionicPickerI18n", function($window) {
        return  {
            ...
            cancel: "取消",
            okClass: "hotlink",
            cancelClass: "warningLink",
            weekdays: $window.moment ? $window.moment.weekdaysMin() : ["日", "一", "二", "三", "四", "五", "六"],
            months: $window.moment ? $window.moment.months() : ["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月"]
        };
    })
    .directive("ionDatetimePicker", function() {
        return {
            ...
            controller: function($scope, $ionicPopup, $ionicPickerI18n, $timeout,pubPopup) {
                ...
                $scope.emptyDate = function(){
                    $scope.empty();
                    $scope.popupShow.close();
                }

                $scope.showPopup = function() {
                    $scope.popupShow = pubPopup.show({
                        templateUrl: "templates/ion-datetime-picker/picker-popup.html",
