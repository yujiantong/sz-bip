angular.module('starter')
  .controller('CustomerDetailsXBZYController',
    ['$scope','$state','$timeout','$ionicPopup','pubHelper','pubPopup','$filter','$ionicModal','$ionicLoading','$ionicHistory','$ionicNavBarDelegate','$localStorage','$ionicViewSwitcher','$stateParams','customerDetailsService',
      function($scope,$state,$timeout,$ionicPopup,pubHelper,pubPopup,$filter,$ionicModal,$ionicLoading,$ionicHistory,$ionicNavBarDelegate,$localStorage,$ionicViewSwitcher,$stateParams,customerDetailsService) {
        var ctrl = this;
        ...
        ctrl.updateAcceptStatus = updateAcceptStatus; //续保专员接收
        ...
        ctrl.agreeReturnBtnSM = agreeReturnBtnSM; //销售经理、服务经理同意回退
        ctrl.refuseReturnBtnSM = refuseReturnBtnSM; //销售经理、服务经理拒绝回退
        ctrl.replacePrincipalSM = replacePrincipalSM; //销售经理、服务经理更换负责人
        ctrl.returnByXSJLZD = returnByXSJLZD; //销售经理、服务经理主动回退
        ctrl.AgreetraceReturn = AgreetraceReturn; //续保主管同意回退
        ctrl.unAgreeTraceReturn = unAgreeTraceReturn;//续保主管拒绝回退
        ctrl.replacePrincipal = replacePrincipal;//续保主管更换负责人
        ctrl.updateReturnStatuToYyq = updateReturnStatuToYyq; //续保主管同意延期
        ctrl.updateReturnStatuToCszt = updateReturnStatuToCszt;//续保主管拒绝延期
        ...
        ctrl.cancelSleep = cancelSleep;//续保主管取消睡眠
        ctrl.cancelApproved = cancelApproved;//续保专员撤销
        // 回退
        function goBack(){
          $ionicHistory.goBack();
        }
          ...
          if (ctrl.customerId != null) {
            ...
            customerDetailsService.findCustomerDetailsByCustomerId(params).then(function (result) {
              if (result.status == 'OK') {
                ...
                ctrl.applyStatu = ctrl.customerDetails.customerAssigns[length].applyStatu;
                ...
                ctrl.customerLevel = ctrl.customerDetails.customerLevel;
        ...
        function updateAcceptStatus(){
          $ionicLoading.show();
          customerDetailsService.updateAcceptStatus(ctrl.customerId,ctrl.principalId,ctrl.principal)
            .then(function (result) {
              $ionicLoading.hide();
              if (result.status == 'OK'&&result.results.content.status=='OK') {
                pubHelper.alert(true,"接收成功");
              } else {
                pubHelper.alert(false,result.results.message);
              }
            });
        ...
        //销售经理、服务经理同意回退
        $ionicModal.fromTemplateUrl('templates/modal/agreeReturn.html', {
          scope: $scope,
          animation: 'slide-in-right'
        }).then(function(modal) {
          $scope.TyhtBox = modal;
        ...
        //销售经理、服务经理同意回退操作按钮
        function agreeReturnBtnSM(){
          if(!ctrl.confirmData.tyhtyj){
            pubHelper.alert(false,"原因不能为空");
            return;
          }
          $ionicLoading.show();
          if(ctrl.user.role.roleId==7){
            customerDetailsService.traceReturnSaM(ctrl.customerId,ctrl.clerkId,ctrl.confirmData.tyhtyj,ctrl.principalId,ctrl.principal)
              .then(function (result) {
                $ionicLoading.hide();
                if (result.status == 'OK' && result.results.content.status == 'OK') {
                  $scope.TyhtBox.hide();
          ...
          }else if(ctrl.user.role.roleId==9){
            customerDetailsService.traceReturnSeM(ctrl.customerId,ctrl.clerkId,ctrl.confirmData.tyhtyj,ctrl.principalId,ctrl.principal)
              .then(function (result) {
                $ionicLoading.hide();
                if (result.status == 'OK' && result.results.content.status == 'OK') {
                  $scope.TyhtBox.hide();
        ...
        //销售经理、服务经理拒绝回退
        $ionicModal.fromTemplateUrl('templates/modal/refuseReturn.html', {
          scope: $scope,
          animation: 'slide-in-right'
        }).then(function(modal) {
          $scope.refuseBox = modal;
        });
        //销售经理、服务经理拒绝回退操作按钮
        function refuseReturnBtnSM(){
          if(!ctrl.confirmData.jjhtyy){
            pubHelper.alert(false,"原因不能为空");
            return;
          }
          $ionicLoading.show();
          if(ctrl.user.role.roleId==7){
            customerDetailsService.unAgreeReturnBySaM(ctrl.customerId,ctrl.clerkId,ctrl.principalId,ctrl.principal,ctrl.confirmData.jjhtyy)
              .then(function (result) {
                $ionicLoading.hide();
                if (result.status == 'OK' && result.results.content.status == 'OK') {
                  $scope.refuseBox.hide();
                  setTimeout(function(){
                    pubHelper.alert(true,"操作成功");
                  },500)
                } else {
            ...
            customerDetailsService.unAgreeReturnBySeM(ctrl.customerId,ctrl.clerkId,ctrl.principalId,ctrl.principal,ctrl.confirmData.jjhtyy)
              .then(function (result) {
                $ionicLoading.hide();
                if (result.status == 'OK' && result.results.content.status == 'OK') {
                  $scope.refuseBox.hide();
                ...
                customerDetailsService.AgreetraceReturn(ctrl.customerId, ctrl.principalId,ctrl.principal,ctrl.tyhtyj)
                  .then(function (result) {
                    ...
                    if (result.status == 'OK'&&result.results.content.status=='OK') {
                      ...
                      ctrl.stateChange('searchList',{userId:ctrl.userId,userName:ctrl.userName,cxdata:ctrl.cxdata});
                    } else {
                      pubHelper.alert(false,result.results.message);
                    }
                  });
        ...
        function cancelSleep() {
          ...
          confirmPopup.then(function(res) {
            if(res) {
              $ionicLoading.show();
              customerDetailsService.saveCancelSleep(ctrl.customerId,ctrl.principalId,ctrl.principal)
                .then(function (result) {
                  $ionicLoading.hide();
                  if (result.status == 'OK'&&result.results.content.status=='OK') {
                    pubHelper.alert(true,"取消睡眠成功",0);
                    ctrl.stateChange('searchList',{userId:ctrl.userId,userName:ctrl.userName,cxdata:ctrl.cxdata});
                  } else {
                    pubHelper.alert(false,result.results.message);
                  }
        ...
        //续保专员撤销
        function cancelApproved(applyStatu) {
          var confirmPopup = pubPopup.confirm({
            title: '撤销',
            template: '是否确认撤销该申请？',
            okText:'是',
          ...
          var condition = {customerId:ctrl.customerId,principalId:ctrl.principalId,
            principal:ctrl.principal,applyStatu:ctrl.applyStatu}
          confirmPopup.then(function(res) {
            if(res) {
              $ionicLoading.show();
              customerDetailsService.cancelReturn(condition)
                .then(function (result) {
                  $ionicLoading.hide();
                  if (result.status == 'OK'&&result.results.content.status=='OK') {
                    pubHelper.alert(true,"撤销成功",0);
                    ctrl.stateChange('approvalList',{userId:ctrl.userId,userName:ctrl.userName,gzcxdata:ctrl.gzcxdata});
                  } else {
                    pubHelper.alert(false,result.results.message);
                  }
                });
